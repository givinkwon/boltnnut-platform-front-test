...
const DropdownArrow2 =
  "/static/images/request/Mobile/MobileStep3/step3_dropdown2.png";
const DropUpArrow2 =
  "/static/images/request/Mobile/MobileStep3/step3_dropup2.png";
...
class MobileStep3Container extends Component {
  ...
  render() {
    ...
    const rows1 = [
      createData(
        "작성일자",
        Proposal.estimate_year +
          "." +
          Proposal.estimate_month +
          "." +
          Proposal.estimate_day,
        ""
      ),
      createData(
        "문서번호",
    ...
    if (DetailQuestion.message.includes(message)) {
      ...
      if (
        ManufactureProcess.totalMinPrice > 0 &&
        ManufactureProcess.MinPrice > 0
      ) {
        rows2.splice(1, 1);
        rows2.pop();
        rows2.pop();
        rows2[3] = createData(
          "금형 견적",
          Math.round(ManufactureProcess.totalMinPrice / 10000) +
            "만원" +
            " ~ " +
            Math.round(ManufactureProcess.totalMaxPrice / 10000) +
            "만원",
          "VAT 미포함"
        );
        rows2[4] = createData(
          "사출 견적",
          Math.round(ManufactureProcess.MinPrice / 10) * 10 +
            "원" +
            " ~ " +
            Math.round(ManufactureProcess.MaxPrice / 10) * 10 +
            "원/개(MOQ 1000개)",
          "VAT 미포함"
        );
      }
      ...
      if (ManufactureProcess.SelectedItem.process != 1) {
        ...
        rows2[3] = createData(
          "생산가",
          Math.round(ManufactureProcess.MinPrice / 100) * 100 + "원" + "/개",
          "VAT 미포함"
        );
      }
    }
    return (
      ...
      >
        ...
        <Card>
                ...
                <TableBody>
                  {rows1.map((row) => (
                    <TableRow className={classes.row} key={row.title}>
                      ...
                      >
                        <Font13
                          style={{ textAlign: "center", color: "#86888c" }}
                        >
                          {row.title}
                        </Font13>
                      </TableCell>
                      <TableCell className={classes.cell} width="144">
                        <Font13 style={{ marginLeft: 20, textAlign: "left" }}>
                          {row.content}
                        </Font13>
                      </TableCell>
          ...
          <ContentBox>
            <ContentHeader>
              요청하신 {estimateData.projectTitle} 제품 개발에
              <br />
              최적화된{" "}
              <span style={{ fontSize: 16, color: "#0933b3" }}>
                {rand2}
              </span>{" "}
              곳의 제조 파트너사가 매칭되었습니다.
            </ContentHeader>
            ...
            <ConsultantBox>
              ...
              <DetailContainer
                style={{ display: showConsultantDetail, paddingTop: 38 }}
              >
                {this.state.arrowChecked != null && (
                  <ConsultantTextBox>
                    <Font16>
                      {this.ConsultantInfo[this.state.arrowChecked].Text1}
                    </Font16>
                    <Font14>
                      {this.ConsultantInfo[this.state.arrowChecked].Text2}
                    </Font14>
                    <Font15
                      fontWeight={500}
                      style={{
                    ...
                    >
                      {this.ConsultantInfo[this.state.arrowChecked].Text3}
                    </Font15>
                  </ConsultantTextBox>
                )}
              </DetailContainer>
            ...
            <Buttonv1
              ...
              style={{ margin: "auto", marginTop: 18, marginBottom: 42 }}
            >
              1:1 컨설팅 신청
            </Buttonv1>
          </ContentBox>
