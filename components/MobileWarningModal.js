...
@inject("Home")
@observer
class MobileWarningModalContainer extends React.Component {
  onClickModal = () => {
    const { Home } = this.props;
    Home.mobile_warning_modal_state = false;
  };
      ...
      <Container>
        <InnerContainer>
          ...
          <Button onClick={() => this.onClickModal()}>
            <Title14 style={{ color: "#0933b3", fontWeight: 500 }}>괜찮아요 모바일로 계속 볼게요</Title14>
          </Button>
        </InnerContainer>
      </Container>
    );
...
export default MobileWarningModalContainer;
