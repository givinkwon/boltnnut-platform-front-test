...
//  @observable possible_main_list = []
  ...
  @action loadPartnerInfo = async (partnerId, callback=null) => {
    ...
    if(this.getPartnerById(partnerId)) { return }
    const requestId = window.location.pathname.split('/').pop()
  ...
  @action loadAnswerDetailPage = async (clientId, requestId, answerId, callback) => {
  ...
  getDevelopBigCategoryById = (id) => {
    ...
    const idx = this.develop_big_categories.findIndex(category => category.id == id)
  ...
  getDevelopCategoryById = (id) => {
    ...
    const idx = this.develop_categories.findIndex(category => category.id == id)
      ...
      .then(res => {
        if(res.data.state === 1) {
          if (answer.state === 0) {
            ...
            this.patchMeetingCount(partner.id, partner.success + 1, partner.fail - 1)
          }
  ...
  @action patchMeetingCount = (partnerId, successCount=null, failCount=null) => {
    const token = localStorage.getItem('token')
    let data = {}
  ...
  @action loadMainCategories = () => {
    ...
    AnswerAPI.getMainCategories(req)
      .then(async (res) => {
        ...
        console.log('loadMainCategories: 카테고리 길이 : ' + this.main_categories.length)
        ...
        while(this.main_categories_next) {
          ...
          await AnswerAPI.getNextPage(req)
            .then(res => {
              this.main_categories = this.main_categories.concat(res.data.results)
              this.main_categories_next = res.data.next
  ...
  @action loadProductCategories = () => {
    ...
    AnswerAPI.getCategories(req)
      .then(async (res) => {
        ...
        console.log('loadProductCategories: 카테고리 길이 : ' + this.categories.length)
  ...
  @action loadDevelopCategories = () => {
    ...
    AnswerAPI.getDevelopCategories(req)
      .then(async (res) => {
        ...
        console.log('loadDevelopCategories: 카테고리 길이 : ' + this.develop_categories.length)
        ...
        while(this.develop_categories_next) {
          ...
          await AnswerAPI.getNextPage(req)
            .then(res => {
              this.develop_categories = this.develop_categories.concat(res.data.results)
              this.develop_categories_next = res.data.next
  ...
  @action loadDevelopBigCategories = () => {
    ...
    AnswerAPI.getDevelopBigCategories(req)
      .then(async (res) => {
        ...
        console.log('loadDevelopBigCategories: 카테고리 길이 : ' + this.develop_big_categories.length)
        ...
        while(this.develop_big_categories_next) {
          ...
          await AnswerAPI.getNextPage(req)
            .then(res => {
              this.develop_big_categories = this.develop_big_categories.concat(res.data.results)
              this.develop_big_categories_next = res.data.next
  ...
  @action loadSubclasses = () => {
    ...
    AnswerAPI.getSubclass(req)
      .then(async (res) => {
        ...
        console.log('loadSubclasses: 카테고리 길이 : ' + this.subclasses.length)
 ...
 //   this.possible_subclass_list = []
    ...
    return CategoryAPI.getSubclass(req)
      .then(async (res) => {
        ...
        subclasses.forEach(subclassObj => {
          subclassObj.subclass = subclassObj.subclass.replace(
            re, `<span style="color: ${PRIMARY};">${value}</span>`
          )
        })
      ...
      //    }
          if(type === 'history') {
            main = this.history_subclass_list.find(main => main.id === subclass.maincategory);
          }
          if(!main) {
      ...
      //      }
            ...
            let category = main.category_set.find(category => category.id === subclass.category);
            ...
            if(!category) {
              ...
              let alreadyExists = category.subclass_set.find(subclassObj => subclassObj.id === subclass.id)
              if(!alreadyExists) {
  ...
  //  this.possible_category_list = []
    ...
    return CategoryAPI.getCategoryMiddle(req)
      ...
      //    }
          if(type === 'history') {
            main = this.history_category_list.find(main => main.id === category.maincategory);
          }
